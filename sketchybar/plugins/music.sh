...
LOVED=$(osascript -l JavaScript -e "Application('Music').currentTrack().loved()"x)
...
if [[ ${#TITLE} -gt 46 ]]; then
  TITLE=$(printf "$(echo $TITLE | cut -c 1-46)…")
fi
...
sketchybar -m --set music_logo icon="$ICON" \
              --set music_info label="${TITLE} - ${ARTIST}" \
